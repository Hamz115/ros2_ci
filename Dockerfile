...
RUN git clone https://github.com/Hamz115/ros2_tests.git
...
ENTRYPOINT ["/bin/bash", "-c", "source /ros2_ws/install/setup.bash && (ros2 launch tortoisebot_bringup bringup.launch.py use_sim_time:=True &) && sleep 10 && (ros2 run tortoisebot_waypoints tortoisebot_action_server &) && sleep 5 && GOAL_X=0.5 GOAL_Y=0.5 colcon test --packages-select tortoisebot_waypoints --event-handler=console_direct+"]